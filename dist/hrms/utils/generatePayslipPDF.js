...
const payslipBreakdown_1 = require("./payslipBreakdown");
const GREEN = "#059669";
...
const generatePayslipPDF = async (payslip) => {
    var _a;
    const pdfDir = path_1.default.join(__dirname, "../../uploads/payslips");
    if (!fs_1.default.existsSync(pdfDir))
        fs_1.default.mkdirSync(pdfDir, { recursive: true });
    const pdfPath = path_1.default.join(pdfDir, `payslip-${payslip._id}.pdf`);
    ...
    const b = (0, payslipBreakdown_1.buildPayslipBreakdown)(payslip);
    const left = 40;
    const right = 555;
    /* ===== HEADER ===== */
    doc.fillColor(GREEN_DARK).font("Helvetica-Bold").fontSize(18).text(((_a = company.name) === null || _a === void 0 ? void 0 : _a.toUpperCase()) || "COMPANY", left, 40);
    doc.fillColor(GRAY).font("Helvetica").fontSize(9);
    if (company.address)
        doc.text(company.address, left, 62, { width: 400 });
    const contactLine = [company.email, company.phone].filter(Boolean).join("  |  ");
    if (contactLine)
        doc.text(contactLine, left, 76, { width: 400 });
    ...
    doc
        ...
        .font("Helvetica")
        .fontSize(8)
        .text(`Present Days = Full Present (${(b.presentDays - 0.5 * b.halfDays).toFixed(0)}) + Holidays (${b.holidayDays}) + Paid Leave (${b.paidLeaveDays.toFixed(2)}) + Half Days x 0.5 (${b.halfDays.toFixed(2)} x 0.5) = ${b.presentDays.toFixed(2)}   |   Overtime = ${b.overtimeHours.toFixed(2)} hrs = ${money(b.overtimeAmount)}`, left + 8, y + 4, { width: right - left - 8 });
    y += 30;
    /* ===== EARNINGS ===== */
    ...
    if (b.overtimeAmount > 0)
        earningRows.push(["Overtime Amount", b.overtimeAmount]);
    if (b.encashmentBonus > 0)
        earningRows.push(["Leave Encashment", b.encashmentBonus]);
    ...
    y += 26;
    const deductionRows = [];
    if (b.professionalTax > 0)
        deductionRows.push(["Professional Tax", b.professionalTax]);
    if (b.pf > 0)
        deductionRows.push(["Provident Fund (PF)", b.pf]);
    if (b.tds > 0)
        deductionRows.push(["Income Tax (TDS)", b.tds]);
    if (b.advance > 0)
        deductionRows.push(["Advance", b.advance]);
    if (b.others > 0)
        deductionRows.push(["Others", b.others]);
    if (b.lopDeductionAmount > 0)
        deductionRows.push([`LOP Deduction (${(b.lopDays - b.unpaidLeaveDays).toFixed(2)} days)`, b.lopDeductionAmount]);
    if (b.unpaidLeaveDeductionAmount > 0)
        deductionRows.push([`Unpaid Leave Deduction (${b.unpaidLeaveDays.toFixed(2)} days)`, b.unpaidLeaveDeductionAmount]);
    doc.font("Helvetica").fontSize(9.5);
    ...
    if (company.stamp) {
        try {
            const stampPath = path_1.default.join(__dirname, "../../", String(company.stamp).replace(/^\//, ""));
            if (fs_1.default.existsSync(stampPath)) {
                const stampSize = 80;
