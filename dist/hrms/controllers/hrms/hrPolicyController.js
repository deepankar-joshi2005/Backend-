...
const HrPolicy_1 = require("../../models/hrms/HrPolicy");
...
const seedHrPolicies = async (_req, res) => {
    try {
        const existing = await HrPolicy_1.HrPolicy.countDocuments();
        if (existing === 0) {
            await HrPolicy_1.HrPolicy.insertMany(DEFAULT_POLICIES);
        }
        const policies = await HrPolicy_1.HrPolicy.find().sort({ no: 1 });
        res.json(policies);
    }
    catch (err) {
...
// Get all HR Policies
const getHrPolicies = async (_req, res) => {
    try {
        let policies = await HrPolicy_1.HrPolicy.find().sort({ no: 1 });
        if (policies.length === 0) {
            await HrPolicy_1.HrPolicy.insertMany(DEFAULT_POLICIES);
            policies = await HrPolicy_1.HrPolicy.find().sort({ no: 1 });
        }
        res.json(policies);
    }
    catch (err) {
...
const uploadHrPolicyDocument = async (req, res) => {
    try {
        const { policyId } = req.params;
        const file = req.file;
        if (!file)
            return res.status(400).json({ message: "No file uploaded" });
        const policy = await HrPolicy_1.HrPolicy.findById(policyId);
        if (!policy)
...
const removeHrPolicyDocument = async (req, res) => {
    try {
        const { policyId } = req.params;
        const policy = await HrPolicy_1.HrPolicy.findById(policyId);
        if (!policy)
