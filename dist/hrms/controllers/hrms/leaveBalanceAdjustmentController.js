"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getAdjustmentHistory = exports.getMyBalances = exports.overrideBalance = exports.getBalance = void 0;
const Leave_1 = __importDefault(require("../../models/hrms/Leave"));
const LeaveType_1 = __importDefault(require("../../models/hrms/LeaveType"));
const LeaveBalanceAdjustment_1 = __importDefault(require("../../models/hrms/LeaveBalanceAdjustment"));
const mongoose_1 = __importDefault(require("mongoose"));
// ================= HELPER: CALCULATE CURRENT BALANCE =================
const getEmployeeBalance = async (employeeId, leaveTypeName) => {
    var _a, _b;
    const leaveTypeDoc = await LeaveType_1.default.findOne({ name: leaveTypeName });
    if (!leaveTypeDoc)
        return 0;
    const empObjectId = new mongoose_1.default.Types.ObjectId(employeeId);
    // 1. Calculate used leaves (APPROVED)
    const usedAgg = await Leave_1.default.aggregate([
        {
            $match: {
                employee: empObjectId,
                leaveType: leaveTypeName,
                status: "APPROVED",
            },
        },
        {
            $group: {
                _id: null,
                total: { $sum: "$totalDays" },
            },
        },
    ]);
    const usedLeaves = ((_a = usedAgg[0]) === null || _a === void 0 ? void 0 : _a.total) || 0;
    // 2. Calculate sum of manual adjustments
    const adjAgg = await LeaveBalanceAdjustment_1.default.aggregate([
        {
            $match: {
                employee: empObjectId,
                leaveType: leaveTypeName,
            },
        },
        {
            $group: {
                _id: null,
                total: { $sum: "$adjustment" },
            },
        },
    ]);
    const totalAdjustments = ((_b = adjAgg[0]) === null || _b === void 0 ? void 0 : _b.total) || 0;
    // Initial balance (maxDays) + adjustments - used
    return leaveTypeDoc.maxDays + totalAdjustments - usedLeaves;
};
// ================= GET CURRENT BALANCE API =================
const getBalance = async (req, res) => {
    try {
        const { employeeId, leaveType } = req.query;
        if (!employeeId || !leaveType) {
            return res.status(400).json({ message: "Employee ID and Leave Type are required" });
        }
        const balance = await getEmployeeBalance(employeeId, leaveType);
        res.json({ balance });
    }
    catch (error) {
        res.status(500).json({ message: "Failed to fetch balance" });
    }
};
exports.getBalance = getBalance;
// ================= OVERRIDE BALANCE (CREATE ADJUSTMENT) =================
const overrideBalance = async (req, res) => {
    try {
        const { employeeId, leaveType, newBalance, reason, effectiveDate } = req.body;
        if (!employeeId || !leaveType || newBalance === undefined || !reason) {
            return res.status(400).json({ message: "All fields are required" });
        }
        const currentBalance = await getEmployeeBalance(employeeId, leaveType);
        const adjustmentValue = newBalance - currentBalance;
        if (adjustmentValue === 0) {
            return res.status(400).json({ message: "New balance is same as current balance" });
        }
        const adjustment = await LeaveBalanceAdjustment_1.default.create({
            employee: employeeId,
            leaveType,
            oldBalance: currentBalance,
            newBalance,
            adjustment: adjustmentValue,
            reason,
            addedBy: req.user.id,
            effectiveDate: effectiveDate || new Date(),
        });
        res.status(201).json({
            message: "Balance adjusted successfully",
            adjustment,
        });
    }
    catch (error) {
        console.error(error);
        res.status(500).json({ message: "Failed to override balance" });
    }
};
exports.overrideBalance = overrideBalance;
// ================= GET MY BALANCES (FOR EMPLOYEE) =================
const getMyBalances = async (req, res) => {
    try {
        const userId = req.user.id;
        const empObjectId = new mongoose_1.default.Types.ObjectId(userId);
        // 1. Get all leave types
        const leaveTypes = await LeaveType_1.default.find({ isActive: true });
        // 2. Get used leaves summary
        const usedAgg = await Leave_1.default.aggregate([
            {
                $match: {
                    employee: empObjectId,
                    status: "APPROVED",
                },
            },
            {
                $group: {
                    _id: "$leaveType",
                    total: { $sum: "$totalDays" },
                },
            },
        ]);
        // 3. Get adjustments summary
        const adjAgg = await LeaveBalanceAdjustment_1.default.aggregate([
            {
                $match: {
                    employee: empObjectId,
                },
            },
            {
                $group: {
                    _id: "$leaveType",
                    total: { $sum: "$adjustment" },
                },
            },
        ]);
        const usedMap = Object.fromEntries(usedAgg.map((u) => [u._id, u.total]));
        const adjMap = Object.fromEntries(adjAgg.map((a) => [a._id, a.total]));
        const summary = leaveTypes.map((lt) => {
            const used = usedMap[lt.name] || 0;
            const adj = adjMap[lt.name] || 0;
            const balance = lt.maxDays + adj - used;
            return {
                leaveType: lt.name,
                maxDays: lt.maxDays,
                used,
                adjustments: adj,
                balance: balance < 0 ? 0 : balance,
            };
        });
        res.json(summary);
    }
    catch (error) {
        console.error(error);
        res.status(500).json({ message: "Failed to fetch leave summary" });
    }
};
exports.getMyBalances = getMyBalances;
// ================= GET ADJUSTMENT HISTORY =================
const getAdjustmentHistory = async (req, res) => {
    try {
        const { employeeId } = req.query;
        const filter = employeeId ? { employee: employeeId } : {};
        const history = await LeaveBalanceAdjustment_1.default.find(filter)
            .populate("employee", "name employeeId")
            .populate("addedBy", "name role")
            .sort({ createdAt: -1 });
        res.json(history);
    }
    catch (error) {
        res.status(500).json({ message: "Failed to fetch adjustment history" });
    }
};
exports.getAdjustmentHistory = getAdjustmentHistory;
