...
export const applyLeave = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const empObjectId = new mongoose.Types.ObjectId(req.user.id as string);
...
export const getTodayTeamLeaves = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const leaves = await Leave.find({
      ...
      fromDate: { $lte: today },
      toDate: { $gte: today },
      ...(!req.user.isSystemAdmin && req.user.role !== ROLES.HRMSAdmin ? { companyId: req.user.companyId } : {}),
    })
      .populate({
        path: "employee",
...
export const updateLeaveStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Multi-tenancy check
    if (!req.user.isSystemAdmin && req.user.role !== ROLES.HRMSAdmin && existingLeave.companyId?.toString() !== req.user.companyId?.toString()) {
